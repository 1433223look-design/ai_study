...
${hasSubQs ? `subResults数组必须有${subQs.length}个元素，对应每个小问。` : 'subResults数组放1个元素即可。'}`;
  ...
  async startTargetedPractice() {
    ...
    // 根据诊断模式构建提示
    const isAllChoice = diagnosticMode === 'choice';
    const typeHint = isAllChoice
      ? '全部出选择题，每题4个选项A/B/C/D'
      : '以选择题为主，可混合少量填空题';

    try {
      let questions = null;

      // 分批生成策略：失败则逐步减少题数和token
      const attempts = [
        { count: diagnosticCount, tokens: Math.min(diagnosticCount * 600, 8000) },
        { count: Math.min(diagnosticCount, 8), tokens: 5000 },
        { count: 5, tokens: 3000 }
      ];
      ...
      for (let i = 0; i < attempts.length; i++) {
        ...
        if (i > 0) {
          btn.textContent = `🔄 正在重试（${attempt.count}题）...`;
        }

        const formatExample = isAllChoice
          ? `[{"question":"题目","options":["A. xx","B. xx","C. xx","D. xx"],"answer":"A","explanation":"解析","topic":"知识点","difficulty":"基础"}]`
          : `[{"question":"题目","options":["A. xx","B. xx","C. xx","D. xx"],"answer":"A","explanation":"解析","topic":"知识点","difficulty":"基础"}]\n填空题不加options字段。`;

        const prompt = `为${this.settings.grade}学生做${subject}查漏补缺诊断测试。

学生情况：${context}
范围：${gradeScope}

要求：
1. 出${attempt.count}道题，${typeHint}
2. 覆盖涉及的所有知识点，每题标明topic字段
3. 难度从基础到综合递进
4. explanation简短精炼
...
${formatExample}`;
        ...
        try {
          const reply = await this.callAI([
            { role: 'system', content: `你是${this.settings.grade}${subject}教师。只返回JSON数组，不说其他话。确保JSON完整闭合。` },
            { role: 'user', content: prompt }
          ], 0.5, attempt.tokens);

          questions = this._parseQuestionJSON(reply);
          if (questions && questions.length > 0) break;
        } catch (innerErr) {
          console.warn(`诊断出题第${i+1}次尝试失败:`, innerErr.message);
          if (i === attempts.length - 1) throw innerErr;
          continue;
        }
      }

      if (!questions || questions.length === 0) {
        throw new Error('AI 多次尝试均未能生成有效题目，请检查网络后重试。\n\n建议：\n1. 确认 API Key 可用\n2. 尝试选择"快速诊断（5题）"\n3. 简化情况描述');
      }
